...
class Dataset
  ...
  def open
    @sheet = Oxcelix::Workbook.new(DATASET_PATH, include: ['Point'], copymerge: true).sheets[0]
    result = parse_header
    if result.failure?
      raise result.error
    end
  ...
  def parse_header
    @header = {}
    field  = %w[coordinate type location voltage]
    field.map do |f|
...
class Database
  ...
  def initialize
    @client            = connect_db
    @coll_transmission = @client[:point]
  end
  ...
  def make_docs(values)
    @docs = []
    values.each do |row|
      la,lo = row[:coordinate].split(",").map &:to_f
      docs << {type: "Feature",
               properties: {
